...
- ⏳ Remaining PII follow-up (b, open for other agents): wire the audit line into the frontend so the user sees *why* the request was blocked (currently just shows the generic message). The `piiGuard: { findings }` payload is already on the wire — both the SSE `error` event and the non-SSE JSON 400 carry it; the frontend just needs to surface it.
