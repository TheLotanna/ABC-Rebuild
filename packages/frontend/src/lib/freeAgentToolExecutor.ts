...
const _backendBase = () => (import.meta as unknown as { env: Record<string, string> }).env.VITE_BACKEND_URL || '';
...
async function executeWriteScratchpad(
  ...
  try {
    ...
    // Validation: Prevent overly large content
    if (contentLength > 50000) {
      return {
        success: false,
        error: `Content too large (${Math.round(contentLength / 1024)}KB). Scratchpad is for SUMMARIES, not raw data. Extract key findings instead.`
      };
    }

    // Validation: Detect raw JSON blobs
    const looksLikeRawJSON = (str: string) => {
    ...
    if (looksLikeRawJSON(content)) {
      return {
        success: false,
        error: "Content appears to be raw JSON data. Scratchpad is for SUMMARIES. Extract and save only the key findings you need."
      };
    }
...
// Execute backend tool via Fastify API
export async function executeEdgeFunctionTool(
  tool: string,
  params: Record<string, unknown>
): Promise<ToolResult> {
  // Map tool name → backend API path
  const toolToPath: Record<string, string> = {
    get_time: "/api/tools/time",
    brave_search: "/api/tools/brave-search",
    google_search: "/api/tools/google-search",
    web_scrape: "/api/tools/web-scrape",
    read_github_repo: "/api/tools/github",
    read_github_file: "/api/tools/github",
    send_email: "/api/tools/email",
    image_generation: "/api/run-nano",
    get_call_api: "/api/tools/api-call",
    post_call_api: "/api/tools/api-call",
    execute_sql: "/api/tools/db",
    read_database_schemas: "/api/tools/db",
    elevenlabs_tts: "/api/tools/tts",
    get_weather: "/api/tools/weather",
    read_zip_contents: "/api/tools/zip",
    read_zip_file: "/api/tools/zip",
    extract_zip_files: "/api/tools/zip",
    pdf_info: "/api/tools/pdf",
    pdf_extract_text: "/api/tools/pdf",
    ocr_image: "/api/tools/ocr",
    pronghorn_post: "/api/tools/pronghorn",
  };

  const apiPath = toolToPath[tool];
  if (!apiPath) {
  ...
  try {
    let body = params;
    if (tool === "get_call_api") body = { ...params, method: "GET" };
    else if (tool === "post_call_api") body = { ...params, method: "POST" };
    else if (tool === "image_generation") body = { prompt: params.prompt, model: params.model || "gemini-3-pro-image-preview" };
    else if (tool === "read_database_schemas") body = { ...params, action: "schemas" };

    const resp = await fetch(`${_backendBase()}${apiPath}`, {
      method: "POST",
    ...
    return { success: false, error: error instanceof Error ? error.message : "Backend tool call failed" };
  }
}
