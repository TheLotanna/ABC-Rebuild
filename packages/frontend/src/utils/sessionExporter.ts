...
//
// `jszip` is dynamic-imported inside `exportSessionToZip()` so it stays
// out of the initial bundle. Type-only import keeps the constructor usage
// readable.
...
export async function exportSessionToZip(options: ExportOptions): Promise<Blob> {
  ...
  // Lazy-load jszip on first export.
