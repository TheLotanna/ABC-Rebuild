// Heavy formatters (`docx` ~845 KB chunk, `jspdf` ~837 KB chunk) are
// dynamic-imported inside the two methods that need them so they stay
// out of the initial bundle. The class itself + markdown parsing are
// kept eagerly imported because they're small and used on the hot path.
//
// The type-only imports below are stripped at compile time (so they
// contribute zero runtime bytes to the chunk) but they let helper
// methods reference `TextRun`, `Paragraph`, `Table`, etc. as types
// without pulling the runtime values into the entry's import graph.
import type {
...
function loadJsPdf(): Promise<JsPdfModule> {
...
export class MarkdownProcessor {
  ...
  // Parse inline formatting within text. `TextRun` is passed in by the
  // caller because docx is loaded lazily — see `generateWordDocument`.
  private parseInlineFormatting(text: string, TextRun: typeof DocxTextRun): DocxTextRun[] {
  ...
  async generateWordDocument(title: string, sections: Array<{ title: string; value: string }>): Promise<Blob> {
    // Dynamic-load docx (~845 KB chunk) on first use only.
    const { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, Table, TableRow, TableCell, WidthType } = await loadDocx();
          ...
          switch (element.type) {
                ...
                children.push(
                  new Paragraph({
                    children: [
                      ...
                      ...this.parseInlineFormatting(listItem.content, TextRun)
                    ],
                    spacing: { after: 100 },
                    ...
                    new TableRow({
                      children: row.map(cell => 
                        new TableCell({
                          children: [
                            new Paragraph({
                              children: this.parseInlineFormatting(cell, TextRun)
                            })
                          ],
                          width: { size: 100 / element.headers!.length, type: WidthType.PERCENTAGE }
                        })
            ...
            case 'paragraph':
              if (element.content.trim()) {
                children.push(
                  new Paragraph({
                    children: this.parseInlineFormatting(element.content, TextRun),
                    spacing: { after: 200 },
                  })
                );
              } else {
  ...
  // Generate PDF with proper markdown rendering. Async now because the
  // jspdf import is lazy — see loadJsPdf().
  async generatePDF(title: string, sections: Array<{ title: string; value: string }>): Promise<jsPDFType> {
